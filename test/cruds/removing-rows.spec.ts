...
describe('Removing rows - checking if its possible', () => {
  ...
  it('no if sheet does not exist', () => {
    ...
    expect(engine.isItPossibleToRemoveRows(1, [{ cellValue: 0 }, { cellValue: 1 }])).toEqual(false)
    expect(engine.isItPossibleToRemoveRows(1.5, [{ cellValue: 0 }, { cellValue: 1 }])).toEqual(false)
    expect(engine.isItPossibleToRemoveRows(-1, [{ cellValue: 0 }, { cellValue: 1 }])).toEqual(false)
    expect(engine.isItPossibleToRemoveRows(NaN, [{ cellValue: 0 }, { cellValue: 1 }])).toEqual(false)
    expect(engine.isItPossibleToRemoveRows(Infinity, [{ cellValue: 0 }, { cellValue: 1 }])).toEqual(false)
    expect(engine.isItPossibleToRemoveRows(-Infinity, [{ cellValue: 0 }, { cellValue: 1 }])).toEqual(false)
  })
  ...
  it('yes if theres an array in place where we remove', () => {
    ...
    expect(engine.isItPossibleToRemoveRows(0, [{ cellValue: 1 }, { cellValue: 1 }])).toEqual(true)
    expect(engine.isItPossibleToRemoveRows(0, [{ cellValue: 1 }, { cellValue: 2 }])).toEqual(true)
    expect(engine.isItPossibleToRemoveRows(0, [{ cellValue: 2 }, { cellValue: 1 }])).toEqual(true)
    expect(engine.isItPossibleToRemoveRows(0, [{ cellValue: 3 }, { cellValue: 1 }])).toEqual(true)
    expect(engine.isItPossibleToRemoveRows(0, [{ cellValue: 4 }, { cellValue: 1 }])).toEqual(true)
  })

  it('yes otherwise', () => {
    const [engine] = HyperFormula.buildFromArray([[]])

    expect(engine.isItPossibleToRemoveRows(0, [{ cellValue: 0 }, { cellValue: 1 }])).toEqual(true)
    expect(engine.isItPossibleToRemoveRows(0, [{ cellValue: 1 }, { cellValue: 1 }])).toEqual(true)
    expect(engine.isItPossibleToRemoveRows(0, [{ cellValue: 1 }, { cellValue: 2 }])).toEqual(true)
  })
})
...
describe('Address dependencies, Case 1: same sheet', () => {
  it('case Aa: absolute dependency above removed row should not be affected', () => {
    ...
    engine.removeRows(0, [{ cellValue: 2 }, { cellValue: 1 }])
  ...
  it('case Rab: relative address should be shifted when only formula is moving', () => {
    ...
    engine.removeRows(0, [{ cellValue: 1 }, { cellValue: 2 }])
  ...
  it('case Rbb: relative address should not be affected when dependency and formula is moving', () => {
    ...
    engine.removeRows(0, [{ cellValue: 0 }, { cellValue: 2 }])
    expect(extractReference(engine, adr('A1'))).toEqual(CellAddress.relative(1, 0))
  })
...
describe('Address dependencies, Case 3: formula in different sheet', () => {
  it('case ARa: relative/absolute dependency below removed row should be shifted ', () => {
    ...
    engine.removeRows(1, [{ cellValue: 1 }, { cellValue: 1 }])
  ...
  it('case ARc: relative/absolute dependency in removed range should be replaced by #REF', () => {
    ...
    engine.removeRows(1, [{ cellValue: 0 }, { cellValue: 1 }])
...
describe('Removing rows - range dependencies, same sheet', () => {
  ...
  it('truncates range by rows from top if topmost rows removed - removing does not have to start with range', () => {
    ...
    engine.removeRows(0, [{ cellValue: 1 }, { cellValue: 3 }])
  ...
  it('truncates range by rows from bottom if bottomest rows removed', () => {
    ...
    engine.removeRows(0, [{ cellValue: 3 }, { cellValue: 2 }])
  ...
  it('truncates range by rows from bottom if bottomest rows removed - removing does not have to end with range', () => {
    ...
    engine.removeRows(0, [{ cellValue: 3 }, { cellValue: 3 }])
  ...
  it('truncates range by rows from bottom if bottomest rows removed - removing does not have to end with range but may start on end', () => {
    ...
    engine.removeRows(0, [{ cellValue: 4 }, { cellValue: 2 }])
...
describe('Removing rows - arrays', () => {
  ...
  it('removing row across array should not change array', () => {
    ...
    engine.removeRows(0, [{ cellValue: 4 }, { cellValue: 1 }])
  ...
  it('it should remove array when removing rows with whole matrix', () => {
    ...
    engine.removeRows(0, [{ cellValue: 2 }, { cellValue: 2 }])
...
describe('Removing rows - range mapping', function() {
  ...
  it('shift ranges in range mapping, whole range', () => {
    ...
    engine.removeRows(0, [{ cellValue: 0 }, { cellValue: 3 }])
...
describe('Removing rows - sheet dimensions', () => {
  it('should do nothing when removed row outside effective sheet', () => {
    ...
    engine.removeRows(0, [{ cellValue: 1 }, { cellValue: 1 }])
    engine.removeRows(0, [{ cellValue: 10 }, { cellValue: 6 }])
  ...
  it('should throw error when trying to remove non positive number of rows', () => {
    ...
    expect(() => engine.removeRows(0, [{ cellValue: 1 }, { cellValue: 0 }])).toThrow(new InvalidArgumentsError('starting row to be smaller than the ending row.'))
  })
  ...
  it('returns changed values', () => {
    ...
    const [changes] = engine.removeRows(0, [{ cellValue: 0 }, { cellValue: 1 }])
...
describe('Removing rows - merge ranges', () => {
  it('should work', () => {
    ...
    verifyRangesInSheet(engine, 0, [{ cellValue: 'A1:C1' }, { cellValue: 'A1:C2' }])

    engine.addRows(0, [{ cellValue: 1 }, { cellValue: 2 }])
    engine.removeRows(0, [{ cellValue: 3 }, { cellValue: 1 }])

    verifyRangesInSheet(engine, 0, [{ cellValue: 'A1:C1' }, { cellValue: 'A1:C2' }, { cellValue: 'A1:C3' }])

    engine.setCellContents({sheet: 0, col: 4, row: 0}, '=SUM(B2:B3)')
    engine.addRows(0, [{ cellValue: 1 }, { cellValue: 2 }])

    verifyRangesInSheet(engine, 0, [{ cellValue: 'B4:B5' }])
  })
  ...
  it('should merge ranges', () => {
    ...
    verifyRangesInSheet(engine, 0, [{ cellValue: 'D1:F1' }, { cellValue: 'D1:F2' }, { cellValue: 'B2:C3' }])
    verifyValues(engine)

    engine.addRows(0, [{ cellValue: 0 }, { cellValue: 2 }])

    verifyRangesInSheet(engine, 0, [{ cellValue: 'D3:F3' }, { cellValue: 'D3:F4' }, { cellValue: 'B4:C5' }])
    verifyValues(engine)

    engine.removeRows(0, [{ cellValue: 3 }, { cellValue: 1 }])

    verifyRangesInSheet(engine, 0, [{ cellValue: 'D3:F3' }, { cellValue: 'B4:C4' }])
    verifyValues(engine)
  })
  ...
  it('Should properly deallocate all nodes', () => {
    ...
    engine.addRows(0, [{ cellValue: 2 }, { cellValue: 2 }])
    engine.removeRows(0, [{ cellValue: 4 }, { cellValue: 1 }])

    verifyRangesInSheet(engine, 0, [{ cellValue: 'B2:C2' }, { cellValue: 'B2:C3' }, { cellValue: 'B2:C4' }])
    verifyValues(engine)
  ...
  it('should merge ranges in proper order', () => {
    ...
    verifyRangesInSheet(engine, 0, [{ cellValue: 'A4:A5' }, { cellValue: 'A4:A4' }])
    verifyValues(engine)
  ...
  it('should merge ranges with subranges in proper order', () => {
    ...
    verifyRangesInSheet(engine, 0, [{ cellValue: 'E1:E1' }, { cellValue: 'E1:E2' }, { cellValue: 'E1:E3' }, { cellValue: 'E1:E4'}])
    verifyValues(engine)

    engine.removeRows(0, [{ cellValue: 2 }, { cellValue: 1 }])

    verifyRangesInSheet(engine, 0, [{ cellValue: 'E1:E1' }, { cellValue: 'E1:E2' }, { cellValue: 'E1:E3' }])
    verifyValues(engine)
  ...
  it('should merge ranges when removing multiple rows', () => {
    ...
    verifyRangesInSheet(engine, 0, [{ cellValue: 'E1:E1' }, { cellValue: 'E1:E3' }])

    engine.removeRows(0, [{ cellValue: 1 }, { cellValue: 2 }])

    verifyRangesInSheet(engine, 0, [{ cellValue: 'E1:E1' }])
    verifyValues(engine)
  ...
  it('should merge ranges when removing multiple rows 2', () => {
    ...
    verifyRangesInSheet(engine, 0, [{ cellValue: 'E1:E1' }, { cellValue: 'E1:E2' }, { cellValue: 'E1:E3' }, { cellValue: 'E1:E4'}, {cellValue: 'E4:E4' }])
