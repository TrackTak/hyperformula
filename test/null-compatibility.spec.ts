...
describe('null compatibility', () => {
  it('should evaluate empty reference to null', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '=A2' }]] }, {evaluateNullToZero: false})
  ...
  it('should evaluate empty reference to 0', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '=A2' }]] }, {evaluateNullToZero: true})
    expect(engine.getCellValue(adr('A1')).cellValue).toEqual(0)
    expect(engine.getCellValue(adr('A2')).cellValue).toBeNull()
  })

  it('should evaluate if to null', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '=IF(TRUE(),A2)' }]] }, {evaluateNullToZero: false})
    expect(engine.getCellValue(adr('A1')).cellValue).toBeNull()
    expect(engine.getCellValue(adr('A2')).cellValue).toBeNull()
  })

  it('should evaluate if to 0', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '=IF(TRUE(),A2)' }]] }, {evaluateNullToZero: true})
    expect(engine.getCellValue(adr('A1')).cellValue).toEqual(0)
    expect(engine.getCellValue(adr('A2')).cellValue).toBeNull()
  })
