...
describe('Events', () => {
  ...
  it('sheetRemoved works', function() {
    ...
    expect(handler).toHaveBeenCalledTimes(1);
    expect(handler).toHaveBeenCalledWith('Sheet2', [
      new ExportedCellChange(
        adr('A1'),
        new CellData(detailedErrorWithOrigin(ErrorType.REF, 'Sheet1!A1'))
      ),
    ]);
  });
  ...
  it('namedExpressionAdded works', () => {
    ...
    expect(handler).toHaveBeenCalledWith('myName', [
      new ExportedNamedExpressionChange('myName', 'foobarbaz'),
    ]);
  });
  ...
  it('valuesUpdated works', () => {
    ...
    expect(handler).toHaveBeenCalledWith([
      new ExportedCellChange(adr('A1'), new CellData(43)),
    ]);
  });
  ...
  it('asyncValuesUpdated works', async () => {
    ...
    expect(handler).toHaveBeenCalledWith([
      new ExportedCellChange(adr('B1'), new CellData(1)),
    ]);
  });
  ...
  it('valuesUpdated may sometimes be triggered even if nothing changed', () => {
    ...
    expect(handler).toHaveBeenCalledTimes(1);
    expect(handler).toHaveBeenCalledWith([
      new ExportedCellChange(adr('A1'), new CellData(42)),
    ]);
  });
  ...
  it('suspension and resuming of evaluation', async () => {
    ...
    expect(handlerResumed).toHaveBeenCalledWith([
      new ExportedCellChange(adr('A1'), new CellData(13)),
    ]);
    expect(handlerAsyncValuesUpdated).toHaveBeenCalledTimes(1);
    expect(handlerAsyncValuesUpdated).toHaveBeenCalledWith([
      new ExportedCellChange(adr('B1'), new CellData(1)),
    ]);
  });
  ...
  it('batching', async () => {
    ...
    await engine.batch(() =>
      engine.setCellContents(adr('A1'), [
        [{ cellValue: '13' }, { cellValue: '=ASYNC_FOO()' }],
      ])
    )[1];
    ...
    expect(handlerResumed).toHaveBeenCalledWith([
      ...
      new ExportedCellChange(
        adr('B1'),
        new CellData(getLoadingError('Sheet1!B1'))
      ),
    ]);
    expect(handlerAsyncValuesUpdated).toHaveBeenCalledTimes(1);
    expect(handlerAsyncValuesUpdated).toHaveBeenCalledWith([
