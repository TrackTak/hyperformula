...
(() => {
  ...
  batch(result,
    () => benchmarkCruds('Sheet A:  change value, add/remove row/column', sheetAGenerator(10000), (engine: HyperFormula) => {
      ...
      engine.addRows(0, [{ cellValue: 5000 }, { cellValue: 1 }])
      engine.removeRows(0, [{ cellValue: 8000 }, { cellValue: 1 }])
    ...
    () => benchmarkCruds('Sheet B: change value, add/remove row/column', sheetBGenerator(5000), (engine: HyperFormula) => {
      engine.setCellContents(adr('A1'), '123')
      engine.addRows(0, [{ cellValue: 2000 }, { cellValue: 1 }])
      engine.removeRows(0, [{ cellValue: 3000 }, { cellValue: 1 }])
      engine.addColumns(0, [{ cellValue: 0 }, { cellValue: 1 }])
      engine.removeColumns(0, [{ cellValue: 0 }, { cellValue: 1 }])
    }, [
