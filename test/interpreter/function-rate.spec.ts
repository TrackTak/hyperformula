...
describe('Function RATE', () => {
  ...
  it('should calculate the correct value with correct arguments and defaults', () => {
    ...
    expect(engine.getCellValue(adr('A2')).cellValue).toBeCloseTo(-0.499692679085513)
    //inconsistency with product #1 (returns different value)
    expect(engine.getCellValue(adr('A3')).cellValue).toBeCloseTo(-0.99009900990099)
    //inconsistency with product #1 (returns value)
    expect(engine.getCellValue(adr('A4')).cellValue).toEqualError(detailedError(ErrorType.NUM))
    //inconsistency with product #1 (returns value)
    expect(engine.getCellValue(adr('A5')).cellValue).toEqualError(detailedError(ErrorType.NUM))
    expect(engine.getCellValue(adr('A6')).cellValue).toEqualError(detailedError(ErrorType.VALUE))
    //inconsistency with product #1 (returns #NUM!)
    expect(engine.getCellValue(adr('A7')).cellValue).toBeCloseTo(-0.8)
  })
})
