...
describe('COUNTBLANK', () => {
  ...
  it('with args', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '=COUNTBLANK(B1, C1)' }, { cellValue: '3.14' }]]})
    expect(engine.getCellValue(adr('A1')).cellValue).toEqual(1)
  })

  it('with range', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '1' }, { cellValue: null }, { cellValue: null }, { cellValue: '=COUNTBLANK(A1:C1)'}]]})
    expect(engine.getCellValue(adr('D1')).cellValue).toEqual(2)
  })

  it('with empty strings', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '' }, { cellValue: null }, { cellValue: null }, { cellValue: '=COUNTBLANK(A1:C1)'}]]})
    expect(engine.getCellValue(adr('D1')).cellValue).toEqual(2)
  })
