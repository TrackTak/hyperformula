...
describe('Function F.INV', () => {
  ...
  it('should return error for arguments of wrong type', () => {
    const [engine] = HyperFormula.buildFromArray([
      [{ cellValue: '=F.INV("foo", 2, 3)' }],
      [{ cellValue: '=F.INV(1, "baz", 3)' }],
      [{ cellValue: '=F.INV(1, 2, "bar")' }],
    ])
  ...
  it('should work', () => {
    const [engine] = HyperFormula.buildFromArray([
      [{ cellValue: '=F.INV(0.1, 1, 1)' }],
      [{ cellValue: '=F.INV(0.9, 2, 2)' }],
    ])

    expect(engine.getCellValue(adr('A1')).cellValue).toBeCloseTo(0.0250856309369253, 6)
    expect(engine.getCellValue(adr('A2')).cellValue).toBeCloseTo(9, 6)
  })

  it('truncates second and third arg', () => {
    const [engine] = HyperFormula.buildFromArray([
      [{ cellValue: '=F.INV(0.1, 1.9, 1)' }],
      [{ cellValue: '=F.INV(0.9, 2, 2.9)' }],
    ])
  ...
  it('checks bounds', () => {
    const [engine] = HyperFormula.buildFromArray([
      [{ cellValue: '=F.INV(0.5, 0.999, 1)' }],
      [{ cellValue: '=F.INV(0.5, 1, 0.999)' }],
      [{ cellValue: '=F.INV(-0.0001, 2, 1)' }],
      [{ cellValue: '=F.INV(1.0001, 2, 1)' }],
    ])
