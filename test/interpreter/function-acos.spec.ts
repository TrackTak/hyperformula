...
describe('Function ACOS', () => {
  it('happy path', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '=ACOS(0.5)' }]]})

    expect(engine.getCellValue(adr('A1')).cellValue).toBeCloseTo(1.0471975511966)
  })

  it('when value not numeric', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '=ACOS("foo")' }]]})

    expect(engine.getCellValue(adr('A1')).cellValue).toEqualError(detailedError(ErrorType.VALUE, ErrorMessage.NumberCoercion))
  })

  it('for 1 (edge)', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '=ACOS(1)' }]]})
  ...
  it('when value too large', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '=ACOS(1.1)' }]]})
  ...
  it('wrong number of arguments', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '=ACOS()' }, { cellValue: '=ACOS(1,-1)' }]]})
