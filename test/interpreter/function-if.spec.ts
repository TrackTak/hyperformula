...
describe('Function IF', () => {
  it('wrong number of arguments', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '=IF(TRUE(), "no", 1, 2)' }, { cellValue: '=IF(1)' }]]})

    expect(engine.getCellValue(adr('A1')).cellValue).toEqualError(detailedError(ErrorType.NA, ErrorMessage.WrongArgNumber))
    expect(engine.getCellValue(adr('B1')).cellValue).toEqualError(detailedError(ErrorType.NA, ErrorMessage.WrongArgNumber))
  })
  it('when value is true', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '=IF(TRUE(), "yes", "no")' }]]})

    expect(engine.getCellValue(adr('A1')).cellValue).toEqual('yes')
  })

  it('when value is false', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '=IF(FALSE(), "yes", "no")' }]]})

    expect(engine.getCellValue(adr('A1')).cellValue).toEqual('no')
  })

  it('coercing empty string', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '' }, { cellValue: '=IF(A1, "yes", "no")' }]]})
    expect(engine.getCellValue(adr('B1')).cellValue).toEqual('no')
  })

  it('when condition is weird type', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '=IF("foo", "yes", "no")' }]]})
  ...
  it('returns error if condition is an error', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '=IF(4/0, "yes", "no")' }]]})

    expect(engine.getCellValue(adr('A1')).cellValue).toEqualError(detailedError(ErrorType.DIV_BY_ZERO))
  })

  it('passes errors', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '=IF(TRUE(), 4/0, "no")' }]]})

    expect(engine.getCellValue(adr('A1')).cellValue).toEqualError(detailedError(ErrorType.DIV_BY_ZERO))
  })

  it('passes subtypes of second arg', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '=IF(TRUE(),B1,C1)' }, { cellValue: '1%' }, { cellValue: '1' }]]})
    expect(engine.getCellValueDetailedType(adr('A1'))).toBe(CellValueDetailedType.NUMBER_PERCENT)
  })

  it('passes subtypes of third arg', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '=IF(FALSE(),B1,C1)' }, { cellValue: '1' }, { cellValue: '1%' }]]})
    expect(engine.getCellValueDetailedType(adr('A1'))).toBe(CellValueDetailedType.NUMBER_PERCENT)
  })

  it('passes correct value when other arg is an error', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '=IF(FALSE(), 4/0, "no")' }]]})

    expect(engine.getCellValue(adr('A1')).cellValue).toEqual('no')
  })

  it('when condition is number', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '=IF(1, "yes", "no")' }]]})

    expect(engine.getCellValue(adr('A1')).cellValue).toEqual('yes')
  })

  it('when condition is logic function', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '=IF(OR(1, FALSE()), "yes", "no")' }]]})

    expect(engine.getCellValue(adr('A1')).cellValue).toEqual('yes')
  })

  it('works when only first part is given', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '=IF(TRUE(), "yes")' }]]})

    expect(engine.getCellValue(adr('A1')).cellValue).toEqual('yes')
  })

  it('works when only first part is given and condition is false', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '=IF(FALSE(), "yes")' }]]})
