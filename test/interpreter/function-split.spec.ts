...
describe('Function SPLIT', () => {
  it('wrong number of arguments', () => {
    const [engine] = HyperFormula.buildFromArray([[{ cellValue: '=SPLIT(1)' }, { cellValue: '=SPLIT("a","b","c")' }]])

    expect(engine.getCellValue(adr('A1'))).toEqualError(detailedError(ErrorType.NA, ErrorMessage.WrongArgNumber))
    expect(engine.getCellValue(adr('B1'))).toEqualError(detailedError(ErrorType.NA, ErrorMessage.WrongArgNumber))
  })
  it('happy path', () => {
    const [engine] = HyperFormula.buildFromArray([[{ cellValue: 'some words' }, { cellValue: '=SPLIT(A1, 0)' }]])

    expect(engine.getCellValue(adr('B1'))).toEqual('some')
  })

  it('bigger index', () => {
    const [engine] = HyperFormula.buildFromArray([[{ cellValue: 'some words' }, { cellValue: '=SPLIT(A1, 1)' }]])

    expect(engine.getCellValue(adr('B1'))).toEqual('words')
  })

  it('when continuous delimeters', () => {
    const [engine] = HyperFormula.buildFromArray([[{ cellValue: 'some  words' }, { cellValue: '=SPLIT(A1, 1)' }, { cellValue: '=SPLIT(A1, 2)' }]])
  ...
  it('coerce first argument to string', () => {
    const [engine] = HyperFormula.buildFromArray([[{ cellValue: '42' }, { cellValue: '=SPLIT(A1, 1)' }]])
  ...
  it('when 2nd arg not a number', () => {
    const [engine] = HyperFormula.buildFromArray([[{ cellValue: 'some words' }, { cellValue: '=SPLIT(A1, "foo")' }]])
  ...
  it('when index arg is not value within bounds', () => {
    const [engine] = HyperFormula.buildFromArray([[{ cellValue: 'some words' }, { cellValue: '=SPLIT(A1, 17)' }, { cellValue: '=SPLIT(A1, -1)' }]])
