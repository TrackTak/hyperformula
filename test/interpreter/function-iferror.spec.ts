...
describe('Function IFERROR', () => {
  ...
  it('when no error', () => {
    const [engine] = HyperFormula.buildFromArray([[{ cellValue: '=IFERROR("abcd", "no")' }]])

    expect(engine.getCellValue(adr('A1')).cellValue).toEqual('abcd')
  })

  it('preserves types of first arg', () => {
    const [engine] = HyperFormula.buildFromArray([[{ cellValue: '=IFERROR(B1, 1)' }, { cellValue: '1%' }]])
  ...
  it('when left-error', () => {
    const [engine] = HyperFormula.buildFromArray([[{ cellValue: '=IFERROR(1/0, "yes")' }]])

    expect(engine.getCellValue(adr('A1')).cellValue).toEqual('yes')
  })

  it('when right-error', () => {
    const [engine] = HyperFormula.buildFromArray([[{ cellValue: '=IFERROR("yes", 1/0)' }]])

    expect(engine.getCellValue(adr('A1')).cellValue).toEqual('yes')
  })

  it('when both-error', () => {
    const [engine] = HyperFormula.buildFromArray([[{ cellValue: '=IFERROR(#VALUE!, 1/0)' }]])

    expect(engine.getCellValue(adr('A1')).cellValue).toEqualError(detailedError(ErrorType.DIV_BY_ZERO))
  })

  it('when range', () => {
    const [engine] = HyperFormula.buildFromArray([[{ cellValue: '=IFERROR("yes", A2:A3)' }]])

    expect(engine.getCellValue(adr('A1')).cellValue).toEqualError(detailedError(ErrorType.VALUE, ErrorMessage.WrongType))
  })

  it('when cycle', () => {
    const [engine] = HyperFormula.buildFromArray([[{ cellValue: '=IFERROR(B1, 1)' }, { cellValue: '=B1' }]])

    expect(engine.getCellValue(adr('A1')).cellValue).toEqual(1)
  })

  it('when left-parsing error', () => {
    const [engine] = HyperFormula.buildFromArray([[{ cellValue: '=IFERROR(B1, 1/0)' }, { cellValue: '=SUM(' }]])
