...
describe('SUM', () => {
  ...
  it('SUM with args', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '=SUM(1, B1)' }, { cellValue: '3.14' }]]})
  ...
  it('range only with empty value', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '' }, { cellValue: '=SUM(A1:A1)' }]]})
    expect(engine.getCellValue(adr('B1')).cellValue).toEqual(0)
  })

  it('range only with some empty values', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '42' }, { cellValue: '' }, { cellValue: '13' }, { cellValue: '=SUM(A1:C1)'}]]})
