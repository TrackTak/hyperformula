...
describe('MIN', () => {
  it('MIN with empty args', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '=MIN()' }]]})
    expect(engine.getCellValue(adr('A1')).cellValue).toEqualError(detailedError(ErrorType.NA, ErrorMessage.WrongArgNumber))
  })

  it('MIN with args', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '=MIN(1, B1)' }, { cellValue: '3.14' }]]})
    expect(engine.getCellValue(adr('A1')).cellValue).toEqual(1)
  })

  it('MIN with range', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '1' }], [{ cellValue: '3' }], [{ cellValue: '2' }], [{ cellValue: '=MIN(A1:A3)' }]]})
    expect(engine.getCellValue(adr('A4')).cellValue).toEqual(1)
  })

  it('MIN with mixed arguments', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '1' }], [{ cellValue: '3' }], [{ cellValue: '2' }], [{ cellValue: '=MIN(4,A1:A3)' }]]})
    expect(engine.getCellValue(adr('A4')).cellValue).toEqual(1)
  })

  it('MIN of strings and number', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: 'foo' }], [{ cellValue: 'bar' }], [{ cellValue: '5' }], [{ cellValue: '=MIN(A1:A3)' }]]})
  ...
  it('MIN of empty value', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '' }, { cellValue: '=MIN(A1)' }]]})
    expect(engine.getCellValue(adr('B1')).cellValue).toEqual(0)
  })

  it('MIN of empty value and some negative number', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '' }, { cellValue: '1' }, { cellValue: '=MIN(A1,B1)' }]]})
