...
describe('Function LOG', () => {
  it('happy path', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '=LOG(4, 2)' }]]})

    expect(engine.getCellValue(adr('A1')).cellValue).toBe(2)
  })

  it('logarithmic base has default', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '=LOG(10)' }]]})
  ...
  it('for zero', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '=LOG(0, 42)' }]]})

    expect(engine.getCellValue(adr('A1')).cellValue).toEqualError(detailedError(ErrorType.NUM, ErrorMessage.ValueSmall))
  })

  it('for negative value', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '=LOG(-42, 42)' }]]})

    expect(engine.getCellValue(adr('A1')).cellValue).toEqualError(detailedError(ErrorType.NUM, ErrorMessage.ValueSmall))
  })

  it('for zero base', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '=LOG(42, 0)' }]]})

    expect(engine.getCellValue(adr('A1')).cellValue).toEqualError(detailedError(ErrorType.NUM, ErrorMessage.ValueSmall))
  })

  it('for 1 base', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '=LOG(42, 1)' }]]})
  ...
  it('wrong number of arguments', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '=LOG()' }, { cellValue: '=LOG(42, 42, 42)' }]]})
