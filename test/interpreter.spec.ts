...
describe('Interpreter', () => {
  it('relative addressing formula', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '42' }, { cellValue: '=A1' }]]})

    expect(engine.getCellValue(adr('B1')).cellValue).toBe(42)
  })

  it('number literal', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '3' }]]})
  ...
  it('ranges - VALUE error when evaluating without context', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '1' }], [{ cellValue: '2' }], [{ cellValue: '=A1:A2' }]]})
  ...
  it('procedures - SUM with bad args', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '=SUM(B1)' }, { cellValue: 'asdf' }]]})
  ...
  it('async procedures - loading', () => {
    ...
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '=ASYNC_FOO()' }]]})

    expect(engine.getCellValue(adr('A1')).cellValue).toEqualError(getLoadingError('Sheet1!A1'))
  })

  it('errors - parsing errors', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '=A1C1' }, { cellValue: '=foo(' }, { cellValue: '=)(asdf' }]]})
  ...
  it('function OFFSET basic use', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '5' }, { cellValue: '=OFFSET(B1, 0, -1)' }, { cellValue: '=OFFSET(A1, 0, 0)' }]]})
  ...
  it('function OFFSET out of range', () => {
    const [engine] = HyperFormula.buildFromArray({ cells: [[{ cellValue: '=OFFSET(A1, -1, 0)' }, { cellValue: '=OFFSET(A1, 0, -1)' }]]})
